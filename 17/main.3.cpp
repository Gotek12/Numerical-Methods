...
class Laguerre
{
  public:
    ...
    compNumber laguerr(vecCompNumbers wartosci)
    {
        ...
        compNumber sq = sqrt((n - 1) * ((n - 1) * pochodna(wartosci, 1) * pochodna(wartosci, 1) - n * pochodna(wartosci, 0) * pochodna(wartosci, 2)));
        compNumber firstP = pochodna(wartosci, 1);
        compNumber mianownik;

        if (abs(firstP + sq) > abs(firstP - sq))
        {
            return wartoscLosowana - (n * pochodna(wartosci, 0)) / (firstP + sq);
        }
        else
        {
            return wartoscLosowana - (n * pochodna(wartosci, 0)) / (firstP - sq);
