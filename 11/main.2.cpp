...
class Lagrange
{
  ...
  public:
    ...
    void lag(double xi[], double yi[])
    {
        ...
        for (double xx = -1.0; xx <= 1.0; xx += 0.001)
        {
            ...
            for (int i = 0; i < N; i++)
            {
                yy += gsl_vector_get(lu(x, y, a, countMat), i) * pow(xx, N - 1 - i);
            }
            printf("%g %g\n", xx, yy);
        }
        ...
        for (int i = 0; i < N; i++)
        {
            cout << setprecision(4) << fixed << gsl_vector_get(lu(x, y, a, countMat), i);
            cout << "x^" << N - 1 - i;
            if (i != N - 1)
            {
                cout << " + ";
  ...
  private:
    //metoda przelicza wektor zawierający współczynniki
    gsl_vector *lu(gsl_vector *x, gsl_vector *y, gsl_vector *a, gsl_matrix *countMat)
    {
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
        ...
        //Choleskim się nie da bo nie mamy pozytywnie okreslonej macierzy
        int tmp;
